...
def recipe_update_payload(
    recipe_json: Dict[str, Any], ingredients: List[str], instructions: List[str]
) -> Dict[str, Any]:
    recipe_json["recipeIngredient"] = [
        RecipeIngredient(note=i).model_dump(exclude_none=True) for i in ingredients
    ]
    recipe_json["recipeInstructions"] = [
        RecipeInstruction(text=i).model_dump(exclude_none=True) for i in instructions
    ]
    return recipe_json
    ...
    def create_recipe(
        ...
        try:
            ...
            return mealie.update_recipe(
                slug, recipe_update_payload(recipe_json, ingredients, instructions)
            )
        except Exception as e:
